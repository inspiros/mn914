...
class ImageFolder:
    r"""An image folder dataset intended for self-supervised learning."""
...
def get_dataloader(data_dir,
    ...
    Otherwise, use a dataset predefined in `torchvision.datasets` submodule by defining the parameters
    `dataset` and `train`.
    """
...
def pil_imgs_from_folder(folder):
    r""" Get all images in the folder as PIL images """
    images = []
    filenames = []
...
def build_optimizer(name, model_params, **optim_params):
    r""" Build optimizer from a dictionary of parameters """
