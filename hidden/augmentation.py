...
from .ops.attacks import DifferentiableJPEGCompress


class RandomDifferentiableJPEG(AugmentationBase2D):
    def __init__(self, p, low=10, high=100,
                 mean: Optional[Tuple[float, ...]] = None,
                 std: Optional[Tuple[float, ...]] = None) -> None:
        super().__init__(p=p)
        self.diff_jpegs = nn.ModuleList(
            [DifferentiableJPEGCompress(qf, mean=mean, std=std) for qf in range(low, high, 10)])
...
class HiddenAug(nn.Module):
    ...
    def __init__(self, img_size, p_crop=0.3, p_blur=0.3, p_jpeg=0.3, p_rot=0.3, p_color_jitter=0.3, p_res=0.3,
        ...
        if p_blur > 0:
            ...
            # augmentations += [blur1]
            blur1 = K.RandomGaussianBlur(kernel_size=(3, 3), sigma=(2.0, 2.0), p=1)  # Gaussian blur σ = 2
            blur2 = K.RandomGaussianBlur(kernel_size=(5, 5), sigma= (4.0, 4.0), p=1) # Gaussian blur σ = 4
            augmentations += [blur1, blur2]
        if p_jpeg > 0:
            diff_jpeg1 = DifferentiableJPEGCompress(50, mean=mean, std=std)  # JPEG50
            diff_jpeg2 = DifferentiableJPEGCompress(80, mean=mean, std=std)  # JPEG80
...
class KorniaAug(nn.Module):
    def __init__(self, degrees=30, crop_scale=(0.2, 1.0), crop_ratio=(3 / 4, 4 / 3), blur_size=17,
        ...
        self.diff_jpeg = RandomDifferentiableJPEG(p=p_diff_jpeg, low=diff_jpeg, mean=mean, std=std)
