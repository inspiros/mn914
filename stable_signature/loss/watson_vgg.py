...
def normalize_tensor(x: torch.Tensor) -> torch.Tensor:
    ...
    n, c, h, w = x.shape
    norm_factor = torch.sqrt(torch.sum(x ** 2, dim=1)).view(n, 1, h, w)
    return x / (norm_factor.expand_as(x) + epsilon(x.dtype))
