...
class InterpolativeUpsamplerReference(nn.Module):
    ...
    def forward(self, x):
        ...
        y = nn.functional.conv_transpose2d(x.view(x.shape[0] * x.shape[1], 1, x.shape[2], x.shape[3]), Kernel, stride=2,
...
class InterpolativeDownsamplerReference(nn.Module):
    ...
    def forward(self, x):
        ...
        y = nn.functional.conv2d(x.view(x.shape[0] * x.shape[1], 1, x.shape[2], x.shape[3]), Kernel, stride=2,
                                 padding=self.FilterRadius)
...
class InplaceUpsamplerReference(nn.Module):
    ...
    def forward(self, x):
        ...
        return nn.functional.conv2d(x.view(x.shape[0] * x.shape[1], 1, x.shape[2], x.shape[3]), Kernel, stride=1,
                                    padding=self.FilterRadius).view(*x.shape)
...
class InplaceDownsamplerReference(nn.Module):
    ...
    def forward(self, x):
        Kernel = self.Kernel.view(1, 1, self.Kernel.shape[0], self.Kernel.shape[1]).to(x.dtype)
        y = nn.functional.conv2d(x.view(x.shape[0] * x.shape[1], 1, x.shape[2], x.shape[3]), Kernel, stride=1,
                                 padding=self.FilterRadius).view(*x.shape)
