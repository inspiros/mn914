...
def main():
    ...
    if params.loss_d is None:
        ...
        F_clf = resnet18(low_resolution=True).to(params.device)
    ...
    for ii_key in range(params.num_keys):
        ...
        while start_iter < params.steps:
            train_stats = train(optimizer, message_loss, critic_loss, distillation_loss,
                                G0, G, D, attack_layer, msg_decoder, img_transform, key, metrics, start_iter + 1,
                                params)
...
def train(optimizer: torch.optim.Optimizer, message_loss: Callable, critic_loss: Callable, distillation_loss: Callable,
    ...
    for it in metric_logger.log_every(range(start_iter, min(start_iter + params.eval_freq, params.steps) + 1),
        ...
        z = torch.randn(params.batch_size, G0.z_dim, device=params.device)  # b z
        label = torch.nn.functional.one_hot(
            torch.randint(0, G0.c_dim, (params.batch_size,), device=params.device), num_classes=G0.c_dim).float()
...
def val(G0: nn.Module, G: nn.Module, msg_decoder: nn.Module, img_transform,
    ...
    for it in metric_logger.log_every(range(1, params.eval_steps + 1), params.log_freq, header):
        # random latent vector
        z = torch.randn(params.batch_size, G0.z_dim, device=params.device, generator=generator)  # b z
        label = torch.nn.functional.one_hot(
            torch.randint(0, G0.c_dim, (params.batch_size,), device=params.device, generator=generator),
            num_classes=G0.c_dim).float()
        # decode latents with original and fine-tuned decoder
        x0 = G0(z, label).float()  # b z -> b c h w
        x_w = G(z, label).float()  # b z -> b c h w
